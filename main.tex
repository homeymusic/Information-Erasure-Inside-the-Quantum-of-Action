...
\begin{figure}[t!]
 ...
 \caption{\textbf{Information erasure inside the quantum of action} (schematic in normalized units $\mu_q=q_0 q_{?}$, $q=q_0\mathbf{Q_0}$, $\Delta p= 2p_0\mathcal{P}$, $\Delta q = 2q_0/\mathcal{P}$). Within the symplectic quantum of action $A_{d\Gamma} = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar$, binary sequences $\{s_{?}\}$ (e.g., \texttt{11010101}) represent informationally distinct but operationally indistinguishable coordinate microstates $\{q_{?}\}$ bounded by the conjugate momentum $\pm 1/\mathcal{P}$. Landauer erasure truncates these high-resolution sequences to a minimal binary sequence $\mathbf{s}^{*}$ (e.g., \texttt{1101}) of length $K(\mathbf{Q}^{*})=|\mathbf{s}^{*}|$. The underlying physical microstates collapse into the macrostate $\mathbf{Q}^{*}$ bounded by the uncertainty $|q_{?}-\mathbf{Q}^{*}|=|\delta_{q}|\le \Delta q$, establishing that information resolution is fundamentally limited by the symplectic action $\pi \hbar$. Consequently, the conversion of lost computational degrees of freedom into thermal entropy over the orthogonality time $\tau_{\perp}$ defines the Landauer action $A_L = K(\mathbf{Q}^{*},\mathbf{P}^{*})\ln(2)k_{B}T\tau_\perp$. This requires that the erasure action $A_{\mathrm{erase}}=\frac{\pi}{4}|\delta_{q}|\Delta p$ be constrained by Landauer’s principle from below and de Gosson's geometric boundary from above: $A_L \le A_{\mathrm{erase}} \le \pi \hbar$.}
