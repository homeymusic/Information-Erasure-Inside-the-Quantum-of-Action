...
\caption{\textbf{Information erasure inside the quantum of action} (schematic in normalized units $\mu_q=q_0 q_{?}$, $q=q_0\mathbf{Q_0}$, $\Delta p= 2p_0\mathcal{P}$, $\Delta q = 2q_0/\mathcal{P}$). Within the symplectic quantum of action $A_{d\Gamma} = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar$, binary sequences $\{s_{?}\}$ (e.g., \texttt{11010101}) represent operationally indistinguishable microstates $\{q_{?}\}$. Landauer erasure truncates these high-resolution sequences to a minimal binary sequence $\mathbf{s}^{*}$ (e.g., \texttt{1101}) of length $K(\mathbf{Q}^{*})=|\mathbf{s}^{*}|$. This process effectively coarse-grains the information, causing the underlying microstates to collapse into the macrostate $\mathbf{Q}^{*}$ where the quantum fluctuation $|\delta_{q}|$ is bounded by the coordinate uncertainty $\Delta q$. This establishes that information resolution is fundamentally limited by the symplectic action $\pi \hbar$. Consequently, the conversion of lost computational degrees of freedom into thermal entropy over the orthogonality time $\tau_{\perp}$ defines the Landauer action $A_L = K(\mathbf{Q}^{*},\mathbf{P}^{*}) \ln(2) k_{B} T \tau_\perp$, requiring that the erasure action $A_{\mathrm{erase}}=\frac{\pi}{4}|\delta_{q}|\Delta p$ be constrained by Landauer’s principle from below and de Gosson's geometric boundary from above: $A_L \le A_{\mathrm{erase}} \le A_{d\Gamma} = \pi \hbar$.}
