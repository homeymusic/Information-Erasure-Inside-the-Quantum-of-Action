...
\begin{abstract}
By maximizing entropy through Landauer information erasure within the quantum of action, we show that the continuum collapses into a discrete phase space. Numerical analysis confirms that this geometry accurately reproduces key quantum phenomena: Wigner quasiprobability marginals, black hole quasinormal modes, and Bell-violating spin correlations. This work establishes quantum phase space as the geometric consequence of information erasure at the entropic limit of action.
\end{abstract}
...
We propose that the total Landauer energy $E_L$ required to erase the informational state of a manifold is a function of its bulk action $\mathcal{A}$, modulated by the topological constraints of the \textbf{de Gosson quantum blob}. To maintain dimensional consistency, the action must be normalized against the fundamental symplectic unit $A_0 = \pi \hbar$. This yields a scaling law where energy is the product of the "blob count" raised to a dimensional index and a characteristic temporal power:

\begin{equation}
E_L \approx \frac{C(n)}{\tau} A_0 \left( \frac{\mathcal{A}}{A_0} \right)^{\frac{n_\perp}{2}}
\end{equation}

\noindent Where:
\begin{itemize}
    \item $\mathcal{A}$ is the total action of the system ($[M L^2 T^{-1}]$).
    \item $A_0 = \pi \hbar$ is the \textbf{de Gosson capacity}, defining the minimum phase-space volume of a rigid quantum blob.
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the quantum of action} (schematic). Within the symplectic quantum of action $\delta_q < \Delta q : A = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar$, all surplus complexity from the many thermodynamically inaccessible microstates $q_?$ specified by a program $s_?$ (e.g., $\texttt{110101010}\dots$), is irreversibly erased, collapsing to the macrostate $\mathrm{Q}$ with the minimal program $\mathbf{s}^*$ (e.g., $\texttt{1101}$) of length $K(Q) = |\mathbf{s}^*|$, that runs on a universal computing machine $\mathcal{U}_{\operatorname{L}}$. This Landauer erasure over a characteristic time $\tau_0$ requires an action $A_L$ bounded by the quantum of action $\pi \hbar \ge A_L \ge k_{B}T\ln (2)K(Q)\tau_0$.}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{landauer_momentum.pdf}
    \caption{\textbf{Landauer energy versus normalized momentum for the quantum harmonic oscillator with transitions highlighted.}}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{distributions.pdf}
    \caption{\textbf{Quantum harmonic oscillator coordinate distributions before, at, and after Landauer phase transitions.}}
    \label{fig:distributions}
\end{figure}

\section{Notation}
The variables used primarily throughout this manuscript are dimensionless values—such as coordinate $\mathcal{L}$, momentum $\mathcal{P}$, energy $\mathcal{E}$, and time $\mathcal{T}$—normalized to their characteristic base units ($q_0, p_0, E_0, t_0$). We define the fundamental \textit{quantum of action} as $A_0 = \frac{\pi}{4} q_0 p_0 = \pi \hbar$, which establishes the phase-space resolution of the system. While the base units ($q_0, p_0$) define the system's characteristic scales, the dynamic state resolutions ($\Delta q, \Delta p$) are captured by the dimensionless parameters $\mathcal{L}$ and $\mathcal{P}$.
...
Characteristic Action: $A_0 = \frac{\pi}{4} q_0 p_0 = \pi \hbar$ \\
Normalized Action: $\mathcal{A} = \mathcal{P}^2 + \mathcal{L}^2$ \\
Action: $A = A_0 \mathcal{A}$ \\
Hamiltonian: $\mathcal{H} = A f = E_0 \mathcal{A} = \pi \hbar\left(\mathcal{P}^2 + \mathcal{L}^2\right)f$
Landauer Universal Computing Machine: $\mathcal{U}_{\operatorname{L}}$ \\
Program Bit String: $s$ \\
Minimal Program: $s^*$ \\
Program Length: $|s|$ \\
Kolmogorov Complexity: $\mathrm{K} = |s^*|$ \\
Shannon Entropy: $\mathrm{H}$ \\
Zurek Entropy: $\mathrm{S}= \mathrm{H} + \mathrm{K}$ \\
Erasure: $\Omega(\mathrm{Q}) = |\{q_?\} \overset{\mathcal{U}_{\operatorname{L}}}{\mapsto} \mathrm{Q}|$ \\
Halt Criteria: $|q_?-\mathrm{Q}| < \frac{\pi}{4} \mathcal{P}^{-1}$ \\
Halt Criteria: $|q_?-\mathrm{Q}| < \mathcal{P}^{-1}$ \\
...
Within this hard action boundary $A$, the entropy of the state is physically constrained by the minimum symplectic volume $A = \pi \hbar$. Because all microstates $\mu_q = q_{0}q_{?}$ within this quantum of action must be operationally indistinguishable ($\delta_{q} < \Delta q \implies \frac{\pi}{4} \delta_{q} \Delta p < \pi \hbar$), it is a thermodynamic necessity that only the macrostate $q_{0}Q$ which maximizes system entropy by minimizing local complexity is physically possible: $\delta_{q} = q_{0}|q_{?} - Q|$. Expressed in terms of the quantum of action with $\Delta p = p_{0}\mathcal{P}$, we have:

$$\left(\frac{\pi}{4} q_{0} p_{0}\right) |q_{?} - Q| \mathcal{P} < \pi \hbar.$$

Recognizing that the characteristic action of the system is $A_{0} = \frac{\pi}{4} q_{0} p_{0} = \pi \hbar$, we arrive at:
...
\begin{equation}
\mathcal{U}_{\mathrm{L}}(q_{?}) \mapsto Q
\end{equation}

which irreversibly erases the knowledge of the inaccessible microstates $\{q_{?} : A = \pi \hbar \}$, mapping them to the one minimal complexity state $K(Q) = |\mathbf{s^*}|$ defined by the minimal program $\mathbf{s^*}$, where $K(Q)$ is the Kolmogorov complexity and $|\mathbf{s^*}|$ is the length of the program's bit string.


\subsection{The Canonical Flow}

The resolution of a physical state is determined either by the probabilistic spread of its conjugate observables, $\sigma _{q}\sigma _{p}\ge \hbar /2$, or by its action boundary $A$ in phase space

$$A=\frac{\pi }{4}\Delta p\Delta q\ge \pi \hbar.$$

Within this action boundary $A$, the entropy of the state is physically constrained by the minimum symplectic volume $A=\pi \hbar$. Because all microstates $\mu_q = q_{0}q_{?}$ within this quantum of action must be operationally indistinguishable $\delta _{q}<\Delta q\implies \frac{\pi }{4}\delta _{q}\Delta p<\pi \hbar$, it is a thermodynamic necessity that only the macrostate $q_{0} \mathrm{Q}$ that maximizes system entropy by minimizing local complexity is physically possible $\delta _{q}=q_{0}|q_{?}-\mathrm{Q}|$. Expressed in terms of the quantum of action with $\Delta p=p_{0}\mathcal{P}$ we have $\left(\frac{\pi }{4}q_{0}p_{0}\right)|q_{?}-\mathrm{Q}|\mathcal{P}<\pi \hbar.$ Recognizing that the characteristic action of the system is $A_{0} = \frac{\pi}{4}q_{0}p_{0} = \pi \hbar$, we arrive at:

\begin{equation}
|q_{?}-\mathrm{Q}|<\mathcal{P}^{-1}.
\end{equation}

This thermodynamic collapse is executed by a universal computing machine we call the Landauer eraser 
\begin{equation}
mathcal{U}_{\mathrm{L}}(q_?) \mapsto \mathrm{Q}
\end{equation}, which irreversibly erases the knowledge of the inaccessible microstates $\{q_? : A = \pi \hbar \}$, mapping them to the one minimal complexity state $K(Q)=|\mathbf{s^{*}}|$ defined by the minimal program $\mathbf{s^{*}}$, where $K(Q)$ is Komogorov complexity and $|\mathbf{s^{*}}|$ is the length of the program's bit string.
...
|q_? - \mathrm{Q}| < \Delta q
\implies
\frac{\pi}{4}\mathcal{L}|q_? - \mathrm{Q}| \Delta p < \pi \hbar
$$
...
$$
\left | \frac{x_?}{x_0} - \frac{a}{b} \right | \pi \hbar P < \pi \hbar
$$
...
$$ \delta_p p_0 \Delta x \ge \pi \hbar \implies \left | \frac{p}{p_0} - \frac{c}{d} \right | x_0 p_0 L \ge \pi \hbar $$
...
\subsection*{The Generalized Scaling Law: Symplectic Holo-Dynamics}
We propose that the total Landauer energy $E_L$ required to erase the informational state of a manifold is a function of its bulk action $\mathcal{A}$, modulated by the topological constraints of the \textbf{de Gosson quantum blob}. To maintain dimensional consistency, the action must be normalized against the fundamental symplectic unit $A_0 = h/2$. This yields a scaling law where energy is the product of the "blob count" raised to a dimensional index and a characteristic temporal power:
...
\begin{itemize}
    \item $\mathcal{A}$ is the total action of the system ($[M L^2 T^{-1}]$).
    \item $A_0 = h/2$ is the \textbf{de Gosson capacity}, defining the minimum phase-space volume of a rigid quantum blob.
    \item $\tau$ is the \textbf{erasure duration} (gate time), often derived from the Margolus-Levitin theorem or Lloyd's limit $\tau \sim \hbar/E$, providing the necessary $T^{-1}$ dimension to convert action to energy.
    \item $n_\perp = n - 1/2$ is the \textbf{transverse reduction index}. This represents the effective dimensionality of the "information horizon" through which the bulk action must be processed during erasure.
    \item $C(n) = \frac{\pi^{n/2}}{2^n \Gamma(n/2 + 1)}$ is the \textbf{geometric coefficient} of a unit $n$-ball, representing the \textbf{symplectic packing efficiency}. It accounts for the volume lost when rigid, non-squeezable blobs are packed into a bounding hypercubic manifold.
\end{itemize}
