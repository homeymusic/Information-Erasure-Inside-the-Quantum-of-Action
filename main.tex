...
\begin{abstract}
The Second Law of Thermodynamics drives systems toward maximum entropy. Applied to the symplectic quantum of action, this imperative maximizes total entropy by erasing surplus information from inaccessible microstates within the minimal Heisenberg cell. The resulting collapse transforms the continuum into a quantized geometry. Through numerical analysis of the harmonic oscillator, we demonstrate that this emergent geometry recovers the marginals of the Wigner quasiprobability distribution. These results suggest that quantized phase space is a thermodynamic necessity, providing quantum mechanics with a geometric correspondence that operates independently from the postulates of probability waves and superposition.
\end{abstract}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the quantum of action.} Within the symplectic quantum of action $\mathcal{A}=h/2$, all surplus bits are erased from the inaccessible microstate $x_?$ having program $s_?=110101010\dots$, leaving the state $x$ with the minimal program $\mathbf{s^*}=1101$ that runs on the Stern-Brocot universal computer $\mathcal{U}_{\operatorname{SB}}$. This reduction constitutes a Landauer erasure with a work cost of $W \ge k_B T \ln(2) |\mathbf{s^*}|$,
    where $|\mathbf{s^*}| = K(x)$ is the Kolmogorov complexity of the
    physical state.}
