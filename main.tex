...
\begin{abstract}
The Second Law of Thermodynamics, when applied to the symplectic quantum of action, mandates the maximization of entropy by erasing surplus information within the minimal Heisenberg cell. This complexity-action constraint collapses the continuum into a quantized geometry. Numerical analysis proves that this quantized geometry recovers the Wigner quasiprobability marginals of the quantum harmonic oscillator, the quasinormal spectral modes of black hole event horizons, and the tomographic probability distributions of spin. Spin simulations violate Bell inequalities, demonstrating that the resultant non-classical correlations arise as a thermodynamic necessity of the information-constrained phase space, providing quantum mechanics with a deterministic geometric foundation.
\end{abstract}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the minimal Heisenberg cell} (schematic). Within the symplectic quantum of action $\delta_q < q_0 : A_0 = \frac{\pi}{4} x_0 p_0 = \frac{h}{2}$, all surplus bits are erased from the thermodynamically inaccessible microstate $q_?$ specified by a program (e.g., $s_? = 110101010\dots$), collapsing to the state $\mathrm{Q}$ with the minimal program (e.g., $\mathbf{s}^* = 1101$) that runs on a universal computing machine $\mathcal{U}_{\operatorname{L}}$. This Landauer reduction constitutes a work cost of $W \ge k_B T \ln(2) |\mathbf{s}^*|$, where $|\mathbf{s}^*| = K(x)$ is the Kolmogorov complexity of the physical state.}
