...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the quantum of action} (schematic). Within the symplectic quantum of action defined by $A_L = \frac{\pi}{4} \Delta q \Delta p \le \pi \hbar$, the algorithmic information of the operationally indistinguishable microstates $\{q_?\}$—specified by binary programs $\{s_?\}$ (e.g., \texttt{110101010\dots})—is irreversibly erased by thermodynamic necessity. The indistinguishable microstates collapse to the macrostate $\mathbf{Q_0} : |q_? - \mathbf{Q_0}| = |\delta_q| \le \Delta q$ defined by the minimal binary program $\mathbf{s_0}$ (e.g., \texttt{1101}) of length $K(\mathbf{Q_0}) = |\mathbf{s_0}|$, running on a universal computing machine $\mathcal{U}_{\mathrm{L}}$. This Landauer erasure over a characteristic time $\tau_L$ requires an action $A_{\mathrm{L}}$ bounded by the symplectic quantum of action $A_L \le \pi \hbar$.}
    \label{fig:erase}
\end{figure}
...
which in turns requires the bifurcation of the action into a conjugate action pair,
