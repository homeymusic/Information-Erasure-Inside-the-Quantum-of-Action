...
Microstate (Initial): $q_?$ \\
Macrostate (Final): $\mathrm{Q} \in \mathbb{Q}$ \\

\\

\noindent
\textbf{The Landauer Universal Computer} \\
Landauer Universal Computer: $\mathcal{U}_{\operatorname{L}}$ \\
Programs Bit String: $s^*$ \\
Erasure: $\mathcal{E}$, \quad $\Omega(\mathrm{Q}) = |\{q_?\} \overset{\mathcal{E}}{\mapsto} \mathrm{Q}|$\\
Criteria: $|q_?-\mathrm{Q}| < \frac{\pi}{4} \Pi^{-1}$
