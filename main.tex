...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the quantum of action} (schematic). Within the symplectic quantum of action defined by $\delta_q \le \Delta q$ such that $A_L = \frac{\pi}{4} \Delta q \Delta p \le \pi \hbar$, the algorithmic information of the operationally indistinguishable microstates $\{q_?\}$—specified by binary programs $\{s_?\}$ (e.g., \texttt{110101010\dots})—is irreversibly erased by thermodynamic necessity. The indistinguishable microstates collapse to the macrostate $\mathbf{Q_0}$ defined by the minimal binary program $\mathbf{s_0}$ (e.g., \texttt{1101}) of length $K(\mathbf{Q_0}) = |\mathbf{s_0}|$, running on a universal computing machine $\mathcal{U}_{\mathrm{L}}$. This Landauer erasure over a characteristic time $\tau_0$ requires an action $A_{\mathrm{L}}$ bounded by the symplectic quantum of action $\pi \hbar$: $k_{B}T\ln(2)K(\mathbf{Q_0})\tau_0 \le A_L \le \pi \hbar$.}
    \label{fig:erase}
\end{figure}
...
\begin{equation}
k_{B}T\ln(2)K(\mathbf{Q_0})\tau_0 \le A_L \le \pi \hbar
\end{equation}

\begin{equation}
q = q_0 |q_{?} - \mathbf{Q_0}| \le \mathcal{P}^{-1}.
\end{equation}

\begin{equation}
p = p_0 |p_{?} - \mathbf{P_0}| \le \mathcal{Q}^{-1}.
\end{equation}
