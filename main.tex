...
\begin{abstract}
By maximizing entropy through Landauer information erasure within de Gosson's quantum of action, we show that continuum states collapse into discrete conjugate observables. Numerical analysis confirms that this dynamic accurately reproduces key quantum phenomena. This work provides an algorithmic-geometric correspondence to quantum mechanics at the entropic limit of action.
\end{abstract}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the quantum of action} (schematic). Within the symplectic quantum of action defined by $\delta_q \le \Delta q$ such that $A_L = \frac{\pi}{4} \Delta q \Delta p \le \pi \hbar$, the algorithmic information of the operationally indistinguishable microstates $\{q_?\}$—specified by binary programs $\{s_?\}$ (e.g., \texttt{110101010\dots})—is irreversibly erased by thermodynamic necessity. The indistinguishable microstates collapse to the macrostate $\mathrm{Q}$ defined by the minimal binary program $\mathbf{s}^*$ (e.g., \texttt{1101}) of length $K(\mathrm{Q}) = |\mathbf{s}^*|$, running on a universal computing machine $\mathcal{U}_{\mathrm{L}}$. This Landauer erasure over a characteristic time $\tau_0$ requires an action $A_{\mathrm{L}}$ bounded by the symplectic quantum of action $\pi \hbar$: $k_{B}T\ln(2)K(\mathrm{Q})\tau_0 \le A_L \le \pi \hbar$.}
    \label{fig:erase}
\end{figure}

\section{Overview}

Landauer erasure over a characteristic time $\tau_0$ requires an action  $A_L$ inside the symplectic quantum of action: 

\begin{equation}
k_{B}T\ln(2)K(\mathrm{Q})\tau_0 \le A_L \le \pi \hbar
\end{equation}

\begin{equation}
|q_{?} - \mathrm{Q}| \le \mathcal{P}^{-1}.
\end{equation}

\begin{equation}
|p_{?} - \mathrm{P}| \le \mathcal{Q}^{-1}.
\end{equation}
...
\section{The Algorithmic Necessity of Conjugate Pairs}
...
\begin{itemize}
    \item \textbf{The "Q Blob" (Position focus):} This blob, which appears horizontally elongated in the attached plots, prioritizes the visibility of position information ($\mathrm{Q}$) by minimizing the information content (complexity $K(\mathrm{P})$) of its conjugate momentum. It satisfies the inequality:
    \[|p_{?} - \mathrm{P}| \le \mathcal{Q}^{-1}\]
    This represents the system when one observes its location.

    \item \textbf{The "P Blob" (Momentum focus):} The vertically elongated blob is its partner. It prioritizes momentum information ($\mathrm{P}$) at the expense of position complexity, satisfying the inequality:
    \[|q_{?} - \mathrm{Q}| \le \mathcal{P}^{-1}\]
    This represents the system when one measures its velocity/momentum.
\end{itemize}
...
Macrostate: $q = \mathrm{Q} \Delta q, \quad p = \mathrm{P} \Delta p$ \\
...
Macrostate: $q = \mathrm{Q} q_0, \quad p = \mathrm{P} p_0$ \\
...
Within this hard action boundary, the entropy of the state is physically constrained by the minimum symplectic volume $A = \pi \hbar$. Because all microstates $\{\mu_q\} = \{q_{?} \Delta q\}$ within this quantum of action must be operationally indistinguishable ($\delta_{q} \le \Delta q \implies \frac{\pi}{4} \delta_{q} \Delta p \le \pi \hbar$), it is a thermodynamic necessity that only the macrostate $\mathrm{Q}\Delta q$ which maximizes system entropy by minimizing local complexity is physically possible: $\delta_{q} = |q_{?} - \mathrm{Q}|\Delta q $, where $q_{?}$ and $\mathrm{Q}$ are normalized and dimensionless. Defining the scale as $\Delta p = p_{0}\mathcal{P}$, we have:

$$\left(\frac{\pi}{4} q_{0} p_{0}\right) |q_{?} - \mathrm{Q}| \mathcal{P} \le \pi \hbar.$$
...
The bound $|q_{?} - \mathrm{Q}| \le \mathcal{P}^{-1}$ establishes a fundamental algorithmic event horizon for the physical state. Within this framework, the normalized momentum $\mathcal{P}$ functions as a computational budget; higher momentum scales provide the necessary action to sustain the algorithmic information required for higher-resolution microstates. Conversely, when the work required to maintain the distinction between a microstate $q_{?}$ and its macrostate $\mathrm{Q}$ exceeds the available action, the Landauer eraser enforces a transition to the state of bounded algorithmic complexity. This thermodynamic collapse effectively truncates the bit-string representation of the state, mapping the indistinguishable sub-resolution information to the minimal complexity program $\mathbf{s^*}$ consistent with the $\pi \hbar$ boundary.
