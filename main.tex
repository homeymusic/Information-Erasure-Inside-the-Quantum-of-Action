...
Total system entropy $\mathcal{S} = \mathrm{H} + \mathrm{K}$ scales with the system's action $A$ normalized by the characteristic action $A_0 = \frac{\pi}{4} q_0 p_0 = \pi \hbar$. Here, $\mathrm{H}$ and $\mathrm{K}$ represent Shannon entropy and Kolmogorov complexity, respectively. 

\begin{equation}
\mathcal{S} \approx C(n) \left( \frac{A}{A_0} \right)^{\frac{n_\perp}{2}}
\end{equation}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the minimal action cell} (schematic). Within the symplectic quantum of action $\delta_q < \Delta q : A = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar$, all surplus complexity from the many thermodynamically inaccessible microstates $q_?$ specified by a program (e.g., $s_? = 110101010\dots$), is irreversibly erased, collapsing to the macrostate $\mathrm{Q}$ with the minimal program (e.g., $\mathbf{s}^* = 1101$) of length $|\mathbf{s}^*|$ that runs on a universal computing machine $\mathcal{U}_{\operatorname{L}}$. This Landauer reduction constitutes a work cost of $W \ge k_B T \ln(2) K(Q)$, where $K(Q) = |\mathbf{s}^*|$ is the Kolmogorov complexity of the physical state.}
\end{figure}
