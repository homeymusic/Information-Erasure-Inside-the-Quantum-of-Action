...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the quantum of action} (schematic). Within the symplectic quantum of action $\delta_q < \Delta q : A = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar$, all surplus complexity from the many thermodynamically inaccessible microstates $q_?$ specified by a program $s_?$ (e.g., $\texttt{110101010}\dots$), is irreversibly erased, collapsing to the macrostate $\mathrm{Q}$ with the minimal program $\mathbf{s}^*$ (e.g., $\texttt{1101}$) of length $|\mathbf{s}^*|$, that runs on a universal computing machine $\mathcal{U}_{\operatorname{L}}$. This Landauer erasure over a characteristic time $\tau_0$ requires an action $A \ge k_{B}T\ln (2)K(Q)\tau_0$, which must be bounded by the quantum of action $A_0 = \pi \hbar$.}
\end{figure}
