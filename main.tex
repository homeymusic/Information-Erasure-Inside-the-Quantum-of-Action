...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the quantum of action} (schematic). Within the symplectic quantum of action $\delta_q < \Delta q : A_0 = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar$, all surplus complexity from the many thermodynamically inaccessible microstates $\{q_?\}$ specified by binary programs $\{s_?\}$ (e.g., $\texttt{110101010}\dots$), is irreversibly erased, collapsing to the macrostate $\mathrm{Q}$ with the minimal binary program $\mathbf{s}^*$ (e.g., $\texttt{1101}$) of length $K(Q) = |\mathbf{s}^*|$, that runs on a universal computing machine $\mathcal{U}_{\operatorname{L}}$. This Landauer erasure over a characteristic time $\tau_0$ requires an action $A_L$ bounded by the symplectic quantum of action $\pi \hbar \ge A_L \ge k_{B}T\ln (2)K(Q)\tau_0$.}
\end{figure}
...
Quantum of Action: $A_0 = \pi \hbar$ \\
Quantum of Action: $A_0 = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar$ \\
...
Binary Program: $s$ \\
Minimal Binary Program: $\mathbf{s^*}$ \\
Binary Program Length: $K(\mathrm{Q}) = |\mathbf{s^*}|$ \\
Erasure Mapping: $\mathcal{U}_{\mathrm{L}} : \{q_?\} \mapsto \mathrm{Q}$ \\
...
$$A_0 = \frac{\pi}{4} \Delta p \Delta q = \pi \hbar.$$

Within this hard action boundary $A$, the entropy of the state is physically constrained by the minimum symplectic volume $A_0 = \pi \hbar$. Because all microstates $\{\mu_q\} = \{q_{0}q_{?}\}$ within this quantum of action must be operationally indistinguishable ($\delta_{q} < \Delta q \implies \frac{\pi}{4} \delta_{q} \Delta p < \pi \hbar$), it is a thermodynamic necessity that only the macrostate $q_{0}\mathrm{Q}$ which maximizes system entropy by minimizing local complexity is physically possible: $\delta_{q} = q_{0}|q_{?} - \mathrm{Q}|$, where $q_{?}$ and $\mathrm{Q}$ are normalized and dimensionless. Defining the scale as $\Delta p = p_{0}\mathcal{P}$, we have:

$$\left(\frac{\pi}{4} q_{0} p_{0}\right) |q_{?} - \mathrm{Q}| \mathcal{P} < \pi \hbar.$$
...
which irreversibly erases the algorithmic information of the inaccessible microstates $\{q_{?} : A_0 = \pi \hbar \}$, mapping them to the one minimal complexity state $K(\mathrm{Q}) = |\mathbf{s^*}|$ defined by the minimal program $\mathbf{s^*}$, where $K(\mathrm{Q})$ is the Kolmogorov complexity and $|\mathbf{s^*}|$ is the length of the program's bit string.


The bound $|q_{?} - \mathrm{Q}| < \mathcal{P}^{-1}$ establishes a fundamental algorithmic event horizon for the physical state. Within this framework, the normalized momentum $\mathcal{P}$ functions as a computational budget; higher momentum scales provide the necessary action to sustain the information density required for higher-resolution microstates. Conversely, when the work required to maintain the distinction between a microstate $q_{?}$ and its macrostate $\mathrm{Q}$ exceeds the available action, the Landauer eraser enforces a transition to the state of minimal algorithmic complexity. This thermodynamic collapse effectively truncates the bit-string representation of the state, mapping the inaccessible sub-resolution information to the minimal complexity program $\mathbf{s^*}$ consistent with the $A_0 = \pi \hbar$ boundary.
