...
\begin{figure}[t!]
    ...
    \caption{\textbf{Information erasure inside the quantum of action} (schematic in normalized units $\mu_q=q_0 q_?$, $q=q_0\mathbf{Q_0}$, $\Delta p= 2p_0\mathcal{P}$, $\Delta q = 2q_0/\mathcal{P}$). Within the symplectic quantum of action defined by \(A_{d\Gamma }=\frac{\pi }{4}\Delta q\Delta p=\pi \hbar \), the binary sequences \(\{s_{?}\}\) (e.g., \texttt{11010101})—which represent informationally distinct but operationally indistinguishable coordinate microstates \(\{q_{?}\}\)—are irreversibly erased. While the underlying physical microstates are bounded by the conjugate momentum \(\pm 1/\mathcal{P}\) and collapse into the macrostate \(\mathbf{Q}^{\mathbf{*}}\) defined by the uncertainty \(|q_{?}-\mathbf{Q}^{\mathbf{*}}|=|\delta _{q}|\le \Delta q\), the Landauer erasure specifically acts upon the information. This process truncates the high-resolution sequences \(\{s_{?}\}\) to the minimal binary sequence \(\mathbf{s}^{\mathbf{*}}\) (e.g., \texttt{1101}) of length \(K(\mathbf{Q}^{\mathbf{*}})=|\mathbf{s}^{\mathbf{*}}|\), establishing that the information resolution is fundamentally bounded by the symplectic action \(\pi \hbar\). Consequently, the conversion of lost computational degrees of freedom into thermal entropy over the orthogonality time \(\tau_{\perp }\) defines the Landauer action \(A_L = K(\mathbf{Q}^{\mathbf{*}},\mathbf{P}^{\mathbf{*}})\ln (2)k_{B}T\tau_\perp\) and requires an erasure action \(A_{\operatorname{erase}}=\frac{\pi }{4}|\delta _{q}|\Delta p\), which is bounded from below by the Landauer action and from above by de Gosson's geometric boundary: \(A_L \le A_{\operatorname{erase}}\le \pi \hbar \).}
...
$$
A_L = k_B T \ln(2) K(Q^*, P^*) \tau_\perp, \quad K(Q^*, P^*) = |s^*_Q| + |s^*_P|
$$

Action of Erasure

$$
A_{\operatorname{erase}} = \frac{\pi}{4} q_0 p_0 |q_?-Q^*||p_? - P^*|
$$


De Gosson's Quantum of Action

$$
A_{d\Gamma} = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar
$$

Erasure with the Quantum of Action

$$
A_L \le A_{\operatorname{erase}} \le A_{d\Gamma}
$$

Margolus-Levitin Orthogonality Time

$$
\tau_\perp T = \frac{\pi \hbar}{2 \ln(2) k_B}
$$

$$
\tau_\perp \le \frac{h}{4E}, \quad E = k_B T \ln(2)
$$

Bekenstein Bound for Quantum of Action
...
$$
R = c \tau_\perp = c \frac{\pi \hbar}{2 \ln(2) k_B}
$$
...
$$
S \le \frac{2 \pi k_B}{\hbar c} c \frac{\pi \hbar}{2 \ln(2) k_B}  k_B T \ln(2) = \pi ^2 \ \text{bits}
$$

Bekenstein Bound for Conjugate Action Quanta Pair
