...
\begin{abstract}
By maximizing entropy through Landauer information erasure within de Gosson's quantum of action, we show that continuum states collapse into discrete conjugate observables. Numerical results confirm that these constraints accurately reproduce key quantum phenomena. This work provides an algorithmic-geometric correspondence to quantum mechanics at the entropic limit of action.
\end{abstract}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within a quantum of action} (schematic). Within a symplectic quantum of action defined by $A_{d\Gamma} = \frac{\pi}{4} \Delta q \Delta p = \pi \hbar$, the algorithmic information of the operationally indistinguishable microstates $\{q_?\}$ for the q-coordinate—specified by binary programs $\{s_?\}$ (e.g., \texttt{110101010\dots})—is irreversibly erased by thermodynamic necessity. The indistinguishable microstates collapse to the macrostate $\mathbf{Q_0} : |q_? - \mathbf{Q_0}| = |\delta_q| \le \Delta q$ defined by the minimal binary program $\mathbf{s_0}$ (e.g., \texttt{1101}) of length $K(\mathbf{Q_0}) = |\mathbf{s_0}|$, running on a universal computing machine $\mathcal{U}_{\mathrm{L}}$. This Landauer erasure over a characteristic time $\tau_L$ requires an action $A_{\mathrm{L}}$ bounded by the symplectic quantum of action $k_{B}T\ln(2)K(\mathbf{Q_0})\tau_L \le A_L \le \pi \hbar$.}
    \label{fig:erase}
\end{figure}

\section{Overview}

The de Gosson quantum of action is defined as 
...
and the sum of those quanta gives the total action of the sytem

$$A_\Gamma = \sum A_{d \Gamma}.$$
...
where $\tau _{\hbar }=\frac{\hbar }{k_{B}T}$ is the Planckian dissipation time. Here, the factor of $\pi $ accounts for the elliptical geometry of the quantum of action, while $\ln (2)$ represents the entropic cost of a single bit erasure. This establishes that the erasure time is a specific algorithmic-geometric scaling of the absolute quantum-thermal speed limit.
...
\section{The Algorithmic Necessity of Conjugate Action Quanta}

\subsection{The Entropic Conflict}
The core equation sets a strict cap on how much algorithmic complexity a single "action blob" can contain:
\[k_{B}T\ln(2)K(\mathbf{Q_0})\tau_L \le A_L \le \pi \hbar\]
If one attempts to place all information for both position and momentum into one blob, they run into the fundamental limit imposed by the Planck constant $\hbar$. A single quantum blob in phase space represents an absolute minimum volume, leaving no room for precise conjugate values.m
...
This identity reveals that the "leap" from a single microstate continuum to a conjugate pair of observables is an algorithmic necessity. Because the information budget for a discrete reality is limited by the \textbf{symplectic capacity} \cite{deGosson2011} of the ``quantum blob'' ($\pi \hbar$), the system cannot resolve both position and momentum within a single geometric orientation. 
...
\begin{itemize}
    \item \textbf{The Q-Blob:} Minimizes position uncertainty by utilizing the algorithmic ``slack'' of the conjugate momentum program.
    \item \textbf{The P-Blob:} Minimizes momentum uncertainty by utilizing the algorithmic ``slack'' of the conjugate position program.
\end{itemize}

This duality effectively maps the bulk algorithmic information into the discrete, erasable bits of our observable world, ensuring that the erasure time adheres to the absolute \textbf{Planckian dissipation limit} $\tau_{\hbar} = \frac{\hbar}{k_B T}$, scaled by the factor $\frac{\pi}{\ln(2)}$ which bridges de Gosson's elliptical geometry with thermodynamic entropy.



\subsection{The Algorithmic-Geometric Solution}
...
\begin{itemize}
    \item \textbf{The "Q Blob" (Position Focus):} This state prioritizes the resolution of position information. To achieve a collapse to a discrete observable $\mathbf{Q_0}$, the system utilizes the algorithmic "slack" of the conjugate momentum, satisfying:
    \begin{equation}
        |q_{?} - \mathbf{Q_0}| \le \mathcal{P}^{-1}
    \end{equation}
    \begin{equation}
    ...
    \item \textbf{The "P Blob" (Momentum Focus):} This state prioritizes the resolution of momentum information. To achieve a collapse to a discrete observable $\mathbf{P_0}$, the system utilizes the algorithmic "slack" of the conjugate position, satisfying:
    \begin{equation}
        |p_{?} - \mathbf{P_0}| \le \mathcal{Q}^{-1}
    \end{equation}
    \begin{equation}
...
The series of plots illustrates this: as the "transition" progresses (representing energy levels of a Quantum Harmonic Oscillator), the two blobs morph while maintaining their duality, confirming their linked nature as an algorithmic necessity.

\subsection{The Origin of Quantum Fluctuations}

We propose that the \textbf{quantum fluctuation}, $\delta_q$, is the physical isomorphism of algorithmic information loss. While traditionally viewed as stochastic noise, $\delta_q$ is revealed here to be the physical coordinate displacement resulting from the erasure of the microstate program $s_?$ into the minimal macrostate program $\mathbf{s_0}$. 

On the algorithmic side, the universe discards a string length $\Delta I = |s_?| - |\mathbf{s_0}|$. On the physical side, this is expressed as the fluctuation:
\begin{equation}
    \delta_q = q_{?} - \mathbf{Q_0}
\end{equation}
...
which irreversibly erases the algorithmic information of the indistinguishable microstates $\{q_{?}\} : \pi \hbar$, mapping them to the one minimal complexity state $K(\mathbf{Q_0}) = |\mathbf{s_0}|$ defined by the minimal program $\mathbf{s_0}$, where $K(\mathbf{Q_0})$ is the Kolmogorov complexity and $|\mathbf{s_0}|$ is the length of the program's binary string.

The bound $|q_{?} - \mathbf{Q_0}| \le \mathcal{P}^{-1}$ establishes a fundamental algorithmic event horizon for the physical state. Within this framework, the normalized momentum $\mathcal{P}$ functions as a computational budget; higher momentum scales provide the necessary action to sustain the algorithmic information required for higher-resolution microstates. Conversely, when the work required to maintain the distinction between a microstate $q_{?}$ and its macrostate $\mathbf{Q_0}$ exceeds the available action, the Landauer eraser enforces a transition to the state of bounded algorithmic complexity. This thermodynamic collapse effectively truncates the bit-string representation of the state, mapping the indistinguishable sub-resolution information to the minimal complexity program $\mathbf{s_0}$ consistent with the $\pi \hbar$ boundary.
