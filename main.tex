...
We define $x_{?}$ as the pre-erasure microstate: the original, fine-grained coordinate of the system. We say that information has been erased during an observation because we will never find out what the state $x_{?}$ was originally. Inside the quantum of action $x_0 p_0 = h/2$ (the coarse-grained minimal Heisenberg cell), $x_{?}$ is operationally indistinguishable from the resulting observed state $x$ such that 
$$|x_{?}-x|\Delta p<h/2.$$ 
...
framework, the transition to a quantized state is a physical search for the 
...
$\{x_? : |x_? - x| \Delta p < h/2 \}$. This mechanism minimizes the complexity of the physical record by selecting the most accessible rational convergent within the resolution bound, effectively forcing the phase-space to emerge as a quantized structure from the thermodynamic limit of information erasure.
...
In the Stern-Brocot implementation of $\mathcal{U}$, the program $b$ required to compute $\phi$ is a non-terminating instruction set $b = (1, 0, 1, 0, \dots)$. This infinite string exposes a critical divergence between the Shannon entropy $H_{Sh}$---the \textit{ensemble average} of information---and the algorithmic entropy $H_{alg}$, which quantifies the physical complexity of the \textit{individual microstate}. 

Following the framework of algorithmic thermodynamics \cite{Ebtekar2025}, we define:
\begin{equation}
    H_{Sh} = -\sum p_i \ln p_i, \quad H_{alg}(x) = |b|
\end{equation}
While the Shannon entropy of the alternating rule is finite and low, the physical Landauer cost of manifesting the specific microstate is driven by the absolute length of the instruction set:
\begin{equation}
    \mathcal{W} = \lim_{|b| \to \infty} |b| k T \ln 2 = \infty
\end{equation}

This distinction is fundamental: whereas Shannon entropy measures our mean ignorance of an ensemble, the algorithmic complexity $|b|$ quantifies the actual thermodynamic weight of the specific microstate. As Zurek established in his formulation of \textit{physical entropy} \cite{Zurek1989}, the total entropy $S_d = H_{alg} + H_{Sh}$ must account for the information already manifested in the record. Since no local thermal reservoir possesses the infinite energy capacity required to finalize a non-terminating program, the constructor $\mathcal{U}$ is forced to \textit{halt} at a finite rational convergent. This thermodynamic barrier proves that irrational coordinates are physically unrealizable, leaving the quantized rational record as the only possible state of the phase-space.
