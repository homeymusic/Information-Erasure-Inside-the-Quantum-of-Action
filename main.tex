...
\begin{abstract}
The Second Law of Thermodynamics applied to the symplectic quantum of action maximizes entropy by erasing surplus information within the minimal Heisenberg cell, manifesting a complexity-action duality. The resulting collapse transforms the continuum into a quantum geometry. Numerical analysis proves that this geometry recovers the Wigner marginals of the quantum harmonic oscillator, the quasinormal spectral modes of black hole event horizons, and the quantized azimuthal projections of spin. Spin simulations also violate Bell's Theorem suggesting that quantum phase space is a thermodynamic necessity, providing quantum mechanics with a geometric correspondence independent of probability waves.
\end{abstract}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Erasure within the minimal Heisenberg cell} (schematic). Within the symplectic quantum of action $\delta_x < x_0 : S_0 = h/2$, all surplus bits are erased from the inaccessible microstate $x_?$ with a program (e.g., $s_? = 110101010\dots$), collapsing to the state $x$ with the minimal program (e.g., $\mathbf{s^*} = 1101$) that runs on a universal computer $\mathcal{U}$. This reduction constitutes a Landauer erasure with a work cost of $W \ge k_B T \ln(2) |\mathbf{s^*}|$, where $|\mathbf{s^*}| = K(x)$ is the Kolmogorov complexity of the physical state.}
