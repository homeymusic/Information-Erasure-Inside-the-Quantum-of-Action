...
\begin{abstract}
The Second Law of Thermodynamics, when applied to the symplectic quantum of action, mandates the maximization of entropy by erasing surplus information within the minimal Heisenberg cell. This complexity-action constraint collapses the continuum into a quantized geometry. Numerical analysis proves that this quantized geometry recovers the Wigner quasiprobability marginals of the quantum harmonic oscillator, the quasinormal spectral modes of black hole event horizons, and the tomographic probability distributions of spin, whose correlations violate Bell inequalities. These results demonstrate that quantum phase space arises when system entropy is bounded by symplectic action.
\end{abstract}
...
Total system entropy $ \mathrm{S} = \mathrm{H} + \mathrm{K}$ must be less than or equal to the system's action $A$ normalized by the quantum of action $A_0=\frac{\pi}{4} \Delta q \Delta p = \frac{h}{2}$.

\begin{equation}
\mathcal{S} \approx \frac{\pi}{4} \left( \frac{\mathcal{A}}{A_0} \right)^{\frac{n_\perp}{2}}
\end{equation}

$\mathrm{H}$ and $\mathrm{K}$ are Shannon entropy and Kolmogorov complexity, respectively. Where $n$ is the physical spatial dimension, $n_{\perp }$ is the transverse reduction. For example, for the 1D QHO, $n=1$ and $n_{\perp } = 1/2$, for the 3D Schwarzschild Horizon $n=3$ and $n_{\perp } = 2$.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{erase.pdf}
    \label{fig:quantum_blob}
    \caption{\textbf{Erasure within the minimal Heisenberg cell} (schematic). Within the symplectic quantum of action $\delta_q < \Delta q : A_0 = \frac{\pi}{4} \Delta q \Delta p = \frac{h}{2}$, all surplus bits are erased from the thermodynamically inaccessible microstate $q_?$ specified by a program (e.g., $s_? = 110101010\dots$), collapsing to the macrostate $\mathrm{Q}$ with the minimal program (e.g., $\mathbf{s}^* = 1101$) of length $|\mathbf{s}^*|$ that runs on a universal computing machine $\mathcal{U}_{\operatorname{L}}$. This Landauer reduction constitutes a work cost of $W \ge k_B T \ln(2) K(Q)$, where $K(Q) = |\mathbf{s}^*|$ is the Kolmogorov complexity of the physical state.}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{entropy_momentum.pdf}
    \caption{\textbf{Total system entropy versus momentum with transitions highlighted.}}
    \label{fig:entropy_momentum}
\end{figure}
...
Physical quantities are represented as dimensionless multiples of their characteristic base units (\(p_{0},q_{0},E_{0},t_{0}\)). We define the fundamental action unit as $A_0=\pi/4 \cdot x_0 p_0 = h/2$. We introduce calligraphic scaling parameters (\(\mathcal{P,L ,E,T, A}\)) to represent the dimensionless scaling factors of these domains. Specifically, \(\mathcal{P}\) denotes the momentum scale, which serves as the reciprocal constraint on the machine's spatial resolution.
...
\section*{Information Erasure and the Holographic Scaling of Action}

In this framework, the emergence of a 1D \textbf{Quantum Harmonic Oscillator (QHO)} is modeled as a discrete erasure process within a phase-space manifold. Following the symplectic geometry of \textbf{de Gosson}, the fundamental unit of phase space is the \textit{quantum blob} with a symplectic capacity $A_0 = h/2$. We define the macrostate coordinate $q$ as the result of a \textbf{Landauer erasure} of a hidden microstate $\mu_q$. 

Our results, based on $10^9$ discrete search realizations in a Stern-Brocot (SB) tree, indicate that the \textbf{Kolmogorov complexity} ($\mathcal{S}$) of the erasure gate follows a power-law scaling relative to the total Hamiltonian action $\mathcal{A}$. We define the \textbf{transverse reduction index} $n_\perp$ as the effective dimensionality of the information horizon. The algorithmic entropy is then bounded by the geometric scaling of the action ratio:

\begin{equation}
\mathcal{S} \approx \frac{\pi}{4} \left( \frac{\mathcal{A}}{A_0} \right)^{\frac{n_\perp}{2}}
\end{equation}

\noindent For the 1D QHO, the erasure maps the 2D symplectic area to a 1D coordinate projection, yielding $n_\perp = 1/2$. This recovers the observed $\mathcal{S} \sim \mathcal{A}^{1/4}$ scaling, representing a highly constrained, sub-holographic regime.

\begin{table*}[t]
\centering
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lcccc|l}
\hline\hline
\textbf{Physical System} & \textbf{Spatial Dim ($n$)} & \textbf{Transverse ($n_\perp$)} & \textbf{Exponent} & \textbf{Growth Law} & \textbf{Information Regime} \\ \hline
1D Emergent QHO          & 1 & 1/2 & 1/4 & $\mathcal{S} \sim \mathcal{A}^{1/4}$ & Sub-Holographic \\
1D Light-Sheet           & 2 & 1   & 1/2 & $\mathcal{S} \sim \mathcal{A}^{1/2}$ & Square-Root Squeeze \\
3D Schwarzschild Horizon & 3 & 2   & 1   & $\mathcal{S} \sim \mathcal{A}^{1}$   & \textbf{Susskind-CA Limit} \\
4D Hyper-Bulk            & 4 & 3   & 3/2 & $\mathcal{S} \sim \mathcal{A}^{3/2}$ & Super-Holographic \\
5D Bulk                  & 5 & 4   & 2   & $\mathcal{S} \sim \mathcal{A}^{2}$   & Quadratic Complexity \\
\hline\hline
\end{tabular*}
\caption{The holographic projection hierarchy: Mapping the physical spatial dimension $n$ to the algorithmic entropy scaling. The transverse reduction $n_\perp = n - 1/2$ defines the effective dimensionality of the information horizon. The 1D case ($n_\perp = 1/2$) reflects the projection of a 2D symplectic blob onto a 1D erasure gate.}
\label{tab:holographic_scaling}
\end{table*}

\subsection*{Operational Emergence in R}
The discrete search is operationalized by enforcing a containment threshold on the microstate fluctuation $\delta_q = |q_? - Q|$ such that $\delta_q < \pi / (4\mathcal{P})$. The emergent macrostate coordinate is then scaled by the Hamiltonian factor $\mathcal{L}^2$, ensuring that the physical coordinate $q$ remains consistent with the energy of the oscillator while the information cost follows the $n_\perp/2$ bound.
